...
`ui_context_menu_run` (`src/widgets/context_menu.asm`, требует `menu_bar.asm` и `window.asm`) открывает popup в стиле dropdown в произвольной точке экрана и возвращает выбранную команду — основа для меню по правой кнопке/контекстных меню.

- `HL` = таблица popup-пунктов (тот же формат, что dropdown `MenuBar`: записи `flags, hotkey, hotkey_mods, command, label_ptr, hint_ptr`, терминатор `UI_MENU_POPUP_END`).
- `D` = x, `E` = y (желаемый верхний левый угол; зажимается, чтобы popup поместился на экране).
- Возвращает `A` = `command` выбранного пункта или `UI_CMD_NONE` при отмене.

Ширина авто-подбирается под самую длинную метку. Popup остаётся открытым после открывающего клика (его отпускание игнорируется) и непрерывно отслеживает мышь: подсветка следует за курсором по пунктам, клик по пункту подтверждает его, клик вне popup закрывает меню. `Up`/`Down` двигают выбор, `Enter`/`Space` или hotkey пункта подтверждают, `Esc` отменяет. Фон под popup сохраняется/восстанавливается через DSS-буфер; используется тот же однопроходный рендер строк dropdown.

`ui_poll_mouse` сообщает о нажатии правой кнопки как `UI_EVENT_RMOUSE` (левая остаётся `UI_EVENT_MOUSE`, так что существующие виджеты не затронуты) и обновляет `ui_event_mouse_x/y` на каждом опросе — открывайте контекстное меню по `UI_EVENT_RMOUSE` в этих координатах. Пример `ctxmenu` (`CTXMENU.EXE`) открывает меню у курсора по правому клику.
...
`ui_text_view` (`src/widgets/text_view.asm`, требует `message_box.asm` для движка переноса и `scrollbar.asm`) — read-only вертикально прокручиваемый просмотр текста в обрамлённой области с постоянно видимым scrollbar. Текст переносится по словам под внутреннюю ширину; `0Ah` принудительно переносит строку.

- Дескриптор: `x, y, width, height, text_ptr (word), top`. `x`/`y` относительны родительскому окну; `width`/`height` включают рамку и колонку scrollbar; `top` — первая видимая строка, поддерживается на месте.
- `ui_draw_text_view` (`IX` = окно, `IY` = дескриптор) рисует; `ui_text_view_run` добавляет модальный цикл: `Up`/`Down` прокручивают на строку (через аппаратный DSS-scroll), `PgUp`/`PgDn`/`Home`/`End` прыгают (перерисовывая видимые строки на месте, без вспышки очистки), стрелки scrollbar реагируют на мышь, `Esc`/`Enter` закрывают.

```asm
help_view:
        db      3, 5, 46, 18            ; x, y, width, height
        dw      help_text              ; ASCIIZ, 0Ah переносит строку
        db      0                      ; top line
...
Виджет хранит только байтовое смещение первой видимой строки: прокрутка вперёд двигает его инкрементально, а полная перерисовка парсит от него (а не с начала текста), поэтому стоимость шага ограничена высотой видимой области, а не длиной документа. Прыжки назад переякоривают это смещение от начала — для типичных текстов просмотрщика это дёшево. Пример `textview` (`TEXTVIEW.EXE`) показывает прокручиваемое окно справки.
...
`ui_file_dialog` (`src/widgets/file_dialog.asm`) — модальный диалог открытия/сохранения из окна, drive-`ComboBox`, `Name`-поля, кнопок Open/Save и Cancel и кастомного **двухколоночного** списка файлов.

```
Вход:  A  = режим (0 = open, 1 = save)
       HL = исходный путь/имя (ASCIIZ; его каталог открывается, имя попадает в поле)
       DE = заголовок (ASCIIZ)
Выход: CF = 0 принято, ui_fd_result = полный путь (ASCIIZ); CF = 1 отмена
```

Записи каталога (`F_First`/`F_Next`) хранятся в **DSS-странице** (выделяется `GetMem`, маппится в WIN3 через `SetWin3`), поэтому основной буфер списка не занимает непрерывную WIN1-память — до ~900 записей при 16-битном счётчике. В WIN1 живут только небольшой staging-срез и индекс. Страница маппится **только** вокруг чистых операций с памятью (`LDIR`, который перед отрисовкой переносит видимые записи в WIN1, и одиночная запись записи при скане); она никогда не удерживается поперёк DSS- или BIOS-вызова, поэтому дисковый I/O и видео-вывод BIOS не могут разрушить маппинг.

Каталоги показываются как `[NAME]` (первым — `[..]`) и открываются через `ChDir` с последующим ре-чтением канонического cwd (диск + `CurDir`), так что строка `Path:` всегда реальна, а неудачный `cd` самокорректируется. Список column-major (вниз по левой колонке, затем по правой); `Up/Down` — на запись, `Left/Right` — на колонку, `Enter` открывает каталог или принимает файл. `Tab` циклирует Name → List → Open → Cancel → Drive. Требует `window.asm`, `text_field.asm`, `combo_box.asm`, `button.asm`, `button_events.asm`, `draw/text.asm` и буфер save-under. Демо: `FILEDLG.EXE`.
